...
`}return n}function handleJPGFiles(e){let t=Array.from(e).filter(e=>"image/jpeg"===e.type||"image/jpg"===e.type||"image/png"===e.type);0!==t.length&&(jpgFiles=[...jpgFiles,...t],renderJPGList())}function renderJPGList(){let e=document.getElementById("jpg-file-list"),t=document.getElementById("jpg-convert-btn");e.innerHTML="",jpgFiles.forEach((t,a)=>{let n=document.createElement("div");n.className="file-item",n.draggable=!0,n.dataset.index=a;let i=document.createElement("div");i.className="file-preview";let r=document.createElement("img");r.src=URL.createObjectURL(t),r.style.width="100%",r.style.height="100%",r.style.objectFit="contain",i.appendChild(r),n.innerHTML=`
    ...
    `,t.classList.remove("hidden"),a.disabled=!1}async function convertWordToPDF(){if(!wordFile)return;let e=document.getElementById("word-convert-btn");try{e.textContent="Preparing...",e.disabled=!0;let t=await wordFile.arrayBuffer(),a=await mammoth.convertToHtml({arrayBuffer:t}),n=a.value,i=a.messages;if(i.length>0&&console.warn("Mammoth warnings:",i),!n)throw Error("No content extracted from Word file.");let r=document.createElement("iframe");r.style.position="absolute",r.style.width="0",r.style.height="0",r.style.border="none",document.body.appendChild(r);let l=r.contentWindow.document;l.open(),l.write(`
        ...
        `),l.close(),showNotification('Print dialog opened. Please select "Save as PDF".',"success")}catch(o){console.error("Word to PDF Error:",o),showNotification("Error processing Word file.","error")}finally{e.textContent="Convert to PDF",e.disabled=!1}}function handleExcelFile(e){if(!e||!e.name.endsWith(".xlsx")&&!e.name.endsWith(".xls"))return;excelFile=e;let t=document.getElementById("excel-file-info"),a=document.getElementById("excel-convert-btn"),n=document.getElementById("excel-upload-area");n.classList.add("hidden"),t.innerHTML=`
        <div class="file-item" style="margin:0; border:var(--primary-color) 1px solid;">
            <div class="file-info">${e.name}</div>
            <div class="file-controls">
    ...
    `)}function updatePasswordStrength(e){let t=document.getElementById("password-strength-bar"),a=document.getElementById("password-strength-text");if(!t||!a)return;if(t.className="strength-bar",!e){t.style.width="0",a.textContent="Password Strength: -";return}let n=0;e.length>=6&&n++,e.length>=10&&n++,/[A-Z]/.test(e)&&n++,/[0-9]/.test(e)&&n++,/[^A-Za-z0-9]/.test(e)&&n++,n<2?(t.classList.add("weak"),t.style.width="20%",a.textContent="Weak"):n<4?(t.classList.add("medium"),t.style.width="60%",a.textContent="Medium"):(t.classList.add("strong"),t.style.width="100%",a.textContent="Strong")}async function protectPDF(){let e=document.getElementById("protect-btn");try{if(!protectFile){showNotification("Please upload a PDF file.","warning");return}let t=document.getElementById("protect-password").value,a=document.getElementById("protect-password-confirm").value;if(!t){showNotification("Please enter a password.","warning");return}if(t!==a){showNotification("Passwords do not match.","error");return}e&&(e.disabled=!0,e.textContent="Encrypting...");let n=await protectFile.arrayBuffer(),i=new Uint8Array(n),r=await getQpdfModule(),l="/input.pdf",o="/output.pdf";try{r.FS.analyzePath("/").exists}catch(s){}try{try{r.FS.unlink(l)}catch(d){}try{r.FS.unlink(o)}catch(c){}r.FS.writeFile(l,i)}catch(g){throw console.error("FS write error:",g),Error("Failed to write file to qpdf virtual FS: "+g.message)}try{r.callMain(["--encrypt",t,t,"256","--",l,o])}catch(f){throw console.error("qpdf callMain error:",f),Error("qpdf failed to encrypt PDF. See console for details.")}let p;try{p=r.FS.readFile(o)}catch(m){throw console.error("qpdf read error:",m),Error("Encrypted output not found. qpdf may have failed.")}let v=new Blob([p],{type:"application/pdf"}),u=`protected_${protectFile.name.replace(/\.pdf$/i,"")}.pdf`;try{triggerDownloadAnimation(e)}catch(h){}try{downloadFile(v,u)}catch(y){let E=URL.createObjectURL(v),w=document.createElement("a");w.href=E,w.download=u,document.body.appendChild(w),w.click(),w.remove(),setTimeout(()=>URL.revokeObjectURL(E),2e3)}showNotification("PDF encrypted successfully!","success");try{r.FS.unlink(l),r.FS.unlink(o)}catch(b){}try{document.getElementById("protect-password").value="",document.getElementById("protect-password-confirm").value="",updatePasswordStrength("")}catch(L){}}catch($){console.error("protectPDF error",$),showNotification("Error protecting PDF. "+($&&$.message?$.message:$),"error")}finally{e&&(e.disabled=!1,e.textContent="Protect & Download PDF")}}window.addStamp=function(e,t){if(!signCanvas)return;let a=new fabric.Group([],{position:"relative",left:250,top:250,originX:"center",originY:"center"}),n=new fabric.Rect({width:200,height:60,fill:"transparent",stroke:t,strokeWidth:5,rx:10,ry:10,originX:"center",originY:"center"}),i=new fabric.Text(e,{fontSize:32,fontFamily:"Impact, Arial Black, sans-serif",fill:t,originX:"center",originY:"center",fontWeight:"bold",charSpacing:50});a.addWithUpdate(n),a.addWithUpdate(i),signCanvas.add(a),signCanvas.setActiveObject(a),signCanvas.requestRenderAll();let r=document.getElementById("stamp-modal");r&&r.classList.add("hidden")},window.removeProtectFile=()=>{protectFile=null;let e=document.getElementById("protect-file-info"),t=document.getElementById("protect-upload-area"),a=document.getElementById("protect-settings-container"),n=document.getElementById("protect-password"),i=document.getElementById("protect-password-confirm"),r=document.getElementById("protect-file-input");r&&(r.value=""),n&&(n.value=""),i&&(i.value=""),updatePasswordStrength(""),a&&a.classList.add("hidden"),t&&t.classList.remove("hidden"),e&&(e.innerHTML="")},"loading"===document.readyState?document.addEventListener("DOMContentLoaded",setupProtectPDFEvents):setupProtectPDFEvents(),document.addEventListener("click",function(e){if(e.target.matches(".toggle-password")||e.target.closest(".toggle-password")){e.preventDefault();let t=e.target.matches(".toggle-password")?e.target:e.target.closest(".toggle-password"),a=t.closest(".password-input-wrapper"),n=a?a.querySelector("input"):null;n?"password"===n.type?(n.type="text",t.textContent="\uD83D\uDC41️"):(n.type="password",t.textContent="\uD83D\uDE48"):(console.log("No input found!"),showNotification("No input found!","warning"))}});let rotateFile=null,rotationStates=[];function handleRotateFile(e){if(!e)return;if("application/pdf"!==e.type){showNotification("Please upload a PDF file","warning");return}rotateFile=e;let t=document.getElementById("rotate-upload-area"),a=document.getElementById("rotate-controls");t&&t.classList.add("hidden"),a&&a.classList.remove("hidden"),renderRotatePreviews(e)}async function renderRotatePreviews(e){let t=document.getElementById("rotate-previews");if(t){t.innerHTML="<p>Loading pages...</p>";try{let a=await e.arrayBuffer(),n=await pdfjsLib.getDocument(a).promise;t.innerHTML="",rotationStates=Array(n.numPages).fill(0);for(let i=1;i<=n.numPages;i++){let r=await n.getPage(i),l=r.getViewport({scale:.3}),o=document.createElement("div");o.className="page-item",o.innerHTML=`
            ...
            `,t.appendChild(o);let s=o.querySelector("canvas");s.height=l.height,s.width=l.width;let d={canvasContext:s.getContext("2d"),viewport:l};r.render(d)}}catch(c){console.error(c),showNotification("Error loading PDF pages","error")}}}async function downloadRotatedPDF(){if(!rotateFile)return;let e=document.getElementById("download-rotated-btn");e.disabled=!0,e.textContent="Processing...";try{let t=await rotateFile.arrayBuffer(),a=await PDFLib.PDFDocument.load(t),n=a.getPages();n.forEach((e,t)=>{let a=rotationStates[t]||0;if(a>0){let n=e.getRotation().angle;e.setRotation(PDFLib.degrees(n+a))}});let i=await a.save();downloadFile(i,`rotated_${rotateFile.name}`),showNotification("Rotated PDF downloaded!","success")}catch(r){console.error(r),showNotification("Failed to save PDF","error")}finally{e.disabled=!1,e.textContent="Download Rotated PDF"}}function setupZoomPan(){let e=document.getElementById("zoom-in"),t=document.getElementById("zoom-out"),a=document.getElementById("zoom-fit");e&&e.addEventListener("click",()=>setZoom(editZoom+.1)),t&&t.addEventListener("click",()=>setZoom(editZoom-.1)),a&&a.addEventListener("click",()=>{editPanX=0,editPanY=0,setZoom(1)});let n=document.getElementById("tool-hand");n&&n.addEventListener("click",()=>setEditTool("hand"));let i=document.getElementById("editor-viewport");if(!i)return;i.addEventListener("mousedown",e=>{"hand"===currentEditTool&&(isPanning=!0,startPanX=e.clientX-editPanX,startPanY=e.clientY-editPanY,i.classList.add("grabbing"))}),window.addEventListener("mousemove",e=>{isPanning&&"hand"===currentEditTool&&(e.preventDefault(),editPanX=e.clientX-startPanX,editPanY=e.clientY-startPanY,applyZoomPanTransforms())}),window.addEventListener("mouseup",()=>{isPanning&&(isPanning=!1,i.classList.remove("grabbing"))}),i.addEventListener("wheel",e=>{if(e.ctrlKey){e.preventDefault();let t=e.deltaY>0?-.1:.1;setZoom(editZoom-t)}},{passive:!1});let r=document.getElementById("zoom-controls");r&&r.classList.remove("hidden")}function setZoom(e){e<.1&&(e=.1),e>5&&(e=5),editZoom=parseFloat(e.toFixed(2));let t=document.getElementById("zoom-level");t&&(t.textContent=Math.round(100*editZoom)+"%"),applyZoomPanTransforms()}function applyZoomPanTransforms(){let e=document.getElementById("editor-transform-layer");e&&(e.style.transform=`translate(${editPanX}px, ${editPanY}px) scale(${editZoom})`)}window.rotatePage=e=>{rotationStates[e]=(rotationStates[e]+90)%360;let t=document.getElementById(`rotate-canvas-${e+1}`);t&&(t.style.transform=`rotate(${rotationStates[e]}deg)`)},document.addEventListener("DOMContentLoaded",()=>{let e=document.getElementById("rotate-file-input"),t=document.getElementById("rotate-upload-area"),a=document.getElementById("download-rotated-btn");e&&e.addEventListener("change",e=>{e.target.files&&e.target.files[0]&&handleRotateFile(e.target.files[0])}),t&&(t.addEventListener("dragover",e=>{e.preventDefault(),t.classList.add("dragover")}),t.addEventListener("dragleave",()=>{t.classList.remove("dragover")}),t.addEventListener("drop",e=>{e.preventDefault(),t.classList.remove("dragover"),e.dataTransfer.files&&e.dataTransfer.files[0]&&handleRotateFile(e.dataTransfer.files[0])})),a&&a.addEventListener("click",downloadRotatedPDF)}),document.addEventListener("DOMContentLoaded",init);let pnState={position:"bc",format:"1",fontSize:12,margin:10,color:"#000000",font:"Helvetica",range:"all",customRange:"",coverPage:!1};const fontUrls={Roboto:"fonts/Roboto-Regular.ttf",OpenSans:"fonts/OpenSans-Regular.ttf",Lato:"fonts/Lato-Regular.ttf",Montserrat:"fonts/Montserrat-Regular.ttf",Oswald:"fonts/Oswald-Regular.ttf",Raleway:"fonts/Raleway-Regular.ttf",Merriweather:"fonts/Merriweather-Regular.ttf",PTSans:"fonts/PTSans-Regular.ttf"};function initPageNumbers(){let e=document.querySelectorAll("#pn-position-grid button");e.forEach(t=>{t.addEventListener("click",t=>{e.forEach(e=>e.classList.remove("active")),t.target.classList.add("active"),pnState.position=t.target.getAttribute("data-pos")})});let t=document.getElementById("pn-format");t&&t.addEventListener("change",e=>{pnState.format=e.target.value});let a=document.getElementById("pn-font-family");a&&a.addEventListener("change",e=>{pnState.font=e.target.value});let n=document.getElementById("pn-fontsize");n&&n.addEventListener("input",e=>{pnState.fontSize=parseInt(e.target.value);let t=document.getElementById("pn-fontsize-val");t&&(t.textContent=pnState.fontSize)});let i=document.getElementById("pn-margin");i&&i.addEventListener("input",e=>{pnState.margin=parseInt(e.target.value);let t=document.getElementById("pn-margin-val");t&&(t.textContent=pnState.margin)});let r=document.getElementById("pn-color");r&&r.addEventListener("input",e=>{pnState.color=e.target.value});let l=document.querySelectorAll("#page-number-controls .wm-toggle button"),o=document.getElementById("pn-custom-range");l.forEach(e=>{e.addEventListener("click",e=>{l.forEach(e=>e.classList.remove("active")),e.target.classList.add("active"),"pn-range-custom"===e.target.id?(pnState.range="custom",o&&o.classList.remove("hidden")):(pnState.range="all",o&&o.classList.add("hidden"))})}),o&&o.addEventListener("input",e=>{pnState.customRange=e.target.value});let s=document.getElementById("pn-cover-page");s&&s.addEventListener("change",e=>{pnState.coverPage=e.target.checked});let d=document.getElementById("apply-page-numbers-btn");d&&(d.onclick=applyPageNumbers)}async function applyPageNumbers(){let e=originalEditFile||editFile;if(!e){showNotification("Please upload a PDF first","warning");return}let t=document.getElementById("apply-page-numbers-btn");t.disabled=!0,t.textContent="Processing...";try{let a=await e.arrayBuffer(),n=await PDFLib.PDFDocument.load(a),i=n.getPages();window.fontkit&&n.registerFontkit(window.fontkit);let r;if(fontUrls[pnState.font])try{let l=await fetch(fontUrls[pnState.font]).then(e=>e.arrayBuffer());r=await n.embedFont(l)}catch(o){console.error("Error loading custom font",o),showNotification("Failed to load font. Using Standard.","warning"),r=await n.embedFont(PDFLib.StandardFonts.Helvetica)}else r="TimesRoman"===pnState.font?await n.embedFont(PDFLib.StandardFonts.TimesRoman):"Courier"===pnState.font?await n.embedFont(PDFLib.StandardFonts.Courier):await n.embedFont(PDFLib.StandardFonts.Helvetica);let s=i.length,d=[];if("all"===pnState.range)d=i.map((e,t)=>t);else{let c=pnState.customRange.split(",");c.forEach(e=>{if((e=e.trim()).includes("-")){let[t,a]=e.split("-").map(e=>parseInt(e));if(!isNaN(t)&&!isNaN(a))for(let n=t;n<=a;n++)n>=1&&n<=s&&d.push(n-1)}else{let i=parseInt(e);!isNaN(i)&&i>=1&&i<=s&&d.push(i-1)}}),d=[...new Set(d)].sort((e,t)=>e-t)}d.forEach(e=>{if(pnState.coverPage&&0===e)return;let t=i[e],{width:a,height:n}=t.getSize(),l=e+1,o=s;pnState.coverPage&&(l=e,o=s-1);let d="";"1"===pnState.format?d=""+l:"page-of"===pnState.format?d="Page "+l+" of "+o:"fraction"===pnState.format&&(d=l+" / "+o);let c=r.widthOfTextAtSize(d,pnState.fontSize),g=r.heightAtSize(pnState.fontSize),f=0,p=0,m=pnState.margin;pnState.position.includes("l")?f=m:pnState.position.includes("c")?f=(a-c)/2:pnState.position.includes("r")&&(f=a-c-m),p=pnState.position.startsWith("t")?n-g-m:m;let v=hexToRgb(pnState.color);t.drawText(d,{x:f,y:p,size:pnState.fontSize,font:r,color:PDFLib.rgb(v.r/255,v.g/255,v.b/255)})});let g=await n.save(),f=new Blob([g],{type:"application/pdf"}),p=new File([f],e.name,{type:"application/pdf"});await handleEditFile(p,!0),showNotification('Page numbers applied. Please review and click "Save & Download PDF" to finish.',"success")}catch(m){console.error(m),showNotification("Error adding page numbers","error")}finally{t.disabled=!1,t.textContent="Add Page Numbers"}}function hexToRgb(e){let t=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(e);return t?{r:parseInt(t[1],16),g:parseInt(t[2],16),b:parseInt(t[3],16)}:{r:0,g:0,b:0}}window.addEventListener("load",()=>{let e=document.getElementById("app-loader");e&&(e.classList.add("loader-hidden"),e.addEventListener("transitionend",()=>{e.remove()}))}),document.addEventListener("DOMContentLoaded",()=>{let e=document.getElementById("tool-sign-text"),t=document.getElementById("sign-text-options"),a=document.getElementById("sign-font-size");a&&a.addEventListener("input",e=>r("fontSize",parseInt(e.target.value,10)));let n=!1,i={fontFamily:"Arial",fontWeight:"normal",fontStyle:"normal",fontSize:20,underline:!1,fill:"#000000"};e&&t&&e.addEventListener("click",a=>{a.stopPropagation();let i=t.classList.contains("hidden");i?(t.classList.remove("hidden"),e.classList.add("active"),n=!0,void 0!==signCanvas&&signCanvas&&(signCanvas.defaultCursor="text")):(t.classList.add("hidden"),e.classList.remove("active"),n=!1,void 0!==signCanvas&&signCanvas&&(signCanvas.defaultCursor="default"))});let r=(e,t)=>{if(void 0===signCanvas||!signCanvas)return;let a=signCanvas.getActiveObject();a&&("i-text"===a.type||"text"===a.type)&&("fontWeight"===e?a.set({fontWeight:t}):"fontStyle"===e?a.set({fontStyle:t}):"underline"===e?a.set({underline:t}):"fontFamily"===e?a.set({fontFamily:t}):"fill"===e?a.set({fill:t}):"fontSize"===e&&a.set({fontSize:parseInt(t,10)}),signCanvas.requestRenderAll()),i[e]=t},l=document.getElementById("sign-font-family");l&&l.addEventListener("change",e=>r("fontFamily",e.target.value));let o=document.getElementById("sign-text-bold");o&&o.addEventListener("click",()=>{let e=o.classList.toggle("active");r("fontWeight",e?"bold":"normal")});let s=document.getElementById("sign-text-italic");s&&s.addEventListener("click",()=>{let e=s.classList.toggle("active");r("fontStyle",e?"italic":"normal")});let d=document.getElementById("sign-text-underline");d&&d.addEventListener("click",()=>{let e=d.classList.toggle("active");r("underline",e)});let c=document.getElementById("sign-text-color");c&&c.addEventListener("input",e=>r("fill",e.target.value));let g=setInterval(()=>{void 0!==signCanvas&&signCanvas&&(clearInterval(g),signCanvas.on("selection:created",f),signCanvas.on("selection:updated",f),signCanvas.on("selection:cleared",p),signCanvas.on("mouse:down",e=>{let a=!t.classList.contains("hidden");if(a&&n&&!e.target){let r=signCanvas.getPointer(e.e),l=new fabric.IText("Type here",{left:r.x,top:r.y,fontFamily:i.fontFamily,fontSize:i.fontSize||20,fill:i.fill,fontWeight:i.fontWeight,fontStyle:i.fontStyle,underline:i.underline});signCanvas.add(l),signCanvas.setActiveObject(l)}}))},1e3);function f(e){let t=e.selected[0];t&&("i-text"===t.type||"text"===t.type)&&(l&&(l.value=t.fontFamily||"Arial"),c&&(c.value=t.fill||"#000000"),o&&o.classList.toggle("active","bold"===t.fontWeight),s&&s.classList.toggle("active","italic"===t.fontStyle),d&&d.classList.toggle("active",!!t.underline),a&&(a.value=t.fontSize||20))}function p(){o&&o.classList.toggle("active","bold"===i.fontWeight),s&&s.classList.toggle("active","italic"===i.fontStyle),d&&d.classList.toggle("active",i.underline),a&&(a.value=i.fontSize||20)}}),document.addEventListener("DOMContentLoaded",()=>{"function"==typeof setupUploadListeners&&setupUploadListeners()}),document.addEventListener("DOMContentLoaded",()=>{var e,t,a;function n(e){let t=e.selected[0];t&&("i-text"===t.type||"text"===t.type)&&(signFontFamily&&(signFontFamily.value=t.fontFamily||"Arial"),signTextColor&&(signTextColor.value=t.fill||"#000000"),signBoldBtn&&signBoldBtn.classList.toggle("active","bold"===t.fontWeight),signItalicBtn&&signItalicBtn.classList.toggle("active","italic"===t.fontStyle),signUnderlineBtn&&signUnderlineBtn.classList.toggle("active",!!t.underline))}function i(){signBoldBtn&&signBoldBtn.classList.toggle("active","bold"===currentTextOptions.fontWeight),signItalicBtn&&signItalicBtn.classList.toggle("active","italic"===currentTextOptions.fontStyle),signUnderlineBtn&&signUnderlineBtn.classList.toggle("active",currentTextOptions.underline)}}),document.addEventListener("DOMContentLoaded",()=>{"function"==typeof setupUploadListeners&&setupUploadListeners()});